<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" :inline="true" v-show="showSearch" label-width="68px">
      <el-form-item label-width="100px" label="入库编号" prop="stockName">
        <el-input
      ...
      <el-form-item label-width="100px" label="供应商" prop="userName">
        <el-input
      ...
      <el-form-item label-width="100px" label="入库仓库" prop="store_name">
        <el-input
    ...
    <el-table v-loading="loading" :data="stockList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="入库编号" align="center" prop="stockName"/>
      <el-table-column label="供货商" align="center" prop="userName"/>
      <!--<el-table-column label="供货商" align="center" prop="stockSupplier"/>-->
      <el-table-column label="入库仓库" align="center" prop="storeName"/>
      <el-table-column label="入库状态" align="center" prop="stockStatus">
        <template slot-scope="scope">
          <dict-tag :options="dict.type.sys_stock_status" :value="scope.row.stockStatus"/>
        </template>
      </el-table-column>
      <el-table-column label="入库类型" align="center" prop="stockType">
        <template slot-scope="scope">
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <!--<el-form-item label-width="100px" label="所属经销商" prop="userId" v-if="roles[0] != 'jingxiaoshang' " >-->
          ...
          <!--</el-select>-->
        <!--</el-form-item>-->
        <el-form-item label-width="100px" label="所属供货商" prop="userId" >
          <el-select v-model="form.userId" placeholder="请选择供货商" ref="supplierSelection">
            <el-option
        ...
        <el-form-item label-width="100px" label="入库状态" prop="stockStatus" v-if="roles[0] != 'jingxiaoshang' && form.stockId" >
          <el-select v-model="form.stockStatus" placeholder="请选择入库状态">
            <el-option
        ...
        <!--<el-form-item label-width="100px" label="入库类型" prop="stockType" >-->
          <!--<el-select v-model="form.stockType" placeholder="请选择入库类型" >-->
            <!--<el-option-->
              <!--v-for="dict in dict.type.sys_stock_type"-->
              <!--v-if="(roles[0] != 'jingxiaoshang' && dict.value=='入库') || dict.value=='入库'"-->
              <!--:key="dict.value"-->
              <!--:label="dict.label"-->
              <!--:value="dict.value"-->
            <!--&gt;</el-option>-->
  ...
  export default {
    ...
    methods: {
        ...
        if(this.form.stockType=='出库'){
          ...
          this.$refs['form'].validate(valid => {
            if (valid) {
              ...
              this.form.userName = this.$refs.supplierSelection.selectedLabel
            }
            if (this.form.stockId != null) {
              updateStock(this.form).then(response => {
                this.$modal.msgSuccess('修改成功')
      ...
      handleDelete(row) {
        ...
        this.$modal.confirm('是否确认删除入库编号为"' + stockIds + '"的数据项？').then(function() {
