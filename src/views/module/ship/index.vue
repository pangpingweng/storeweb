<template>
  <div class="app-container">
    ...
    <el-form-item label-width="90px" label="司机名称" prop="driverId">
        <el-select v-model="form.driverId" placeholder="请选择司机名称" ref="driverSelection">
            <el-option
                    v-for="item in driverList"
                    v-if="item.work_type == '在职'"
                    :key="item.driverId"
    ...
    <el-form-item label-width="90px" label="车辆名称" prop="carId">
        <el-select v-model="form.carId" placeholder="请选择车辆名称" ref="carSelection">
            <el-option
                    v-for="item in carList"
                    v-if="item.car_status == '空闲'"
                    :key="item.carId"
                    :label="item.carName"
                    :value="item.carId"
            ></el-option>
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          ...
          this.driverList.forEach(driver=>{
            if(driver.driverId ==  this.form.driverId){
              that.form.driverphone = driver.phone;
            }
          })
